...
from src.interfaces.interface_adapter.interface_servico_adapter import RouteInterfaceServico as Route
from src.presenters.helpers.http_models import HttpRequest, HttpResponse
from src.presenters.erros.http_erros import HttpErrors


class AdapterServico:
    ...
    def insert_adapter(self):
        if "nome_servico" and "descricao_servico" in self.query_data:
    ...
    def select_by_id_adapter(self):
        if "id_servico" in self.query_data:
            http_request = HttpRequest(query=self.data)
    ...
    def update_adapter(self):
        ...
        if "nome_servico" and "descricao_servico" and "id_servico" in self.query_data:
            http_request = HttpRequest(query=self.data)
