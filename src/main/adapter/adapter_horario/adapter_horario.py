...
from src.interfaces.interface_adapter.interface_horario_adapter import RouteInterfaceHorario as Route
from src.presenters.helpers.http_models import HttpRequest, HttpResponse
from src.presenters.erros.http_erros import HttpErrors


class AdapterHorario:
    ...
    def insert_adapter(self):
        if "horario" and "status" in self.query_data:
    ...
    def select_by_id_adapter(self):
        if "id_horario" in self.query_data:
            http_request = HttpRequest(query=self.data)
    ...
    def select_by_status_adapter(self):
        if "status" in self.query_data:
            http_request = HttpRequest(query=self.data)
            response = self.api_route.route_select_by_status(http_request)
            return response
