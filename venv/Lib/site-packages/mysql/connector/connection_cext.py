...
class CMySQLConnection(MySQLConnectionAbstract):
    ...
    def cursor(
        ...
        """Instantiates and returns a cursor.py using C Extension

        By default, CMySQLCursor is returned. Depending on the options
        while connecting, a buffered and/or raw cursor.py is instantiated
        instead. Also depending upon the cursor.py options, rows can be
        returned as dictionary or named tuple.

        Dictionary and namedtuple based cursors are available with buffered
        output but not raw.

        It is possible to also give a custom cursor.py through the
        cursor_class parameter, but it needs to be a subclass of
        mysql.connector.cursor_cext.CMySQLCursor.

        Raises ProgrammingError when cursor_class is not a subclass of
        CursorBase. Raises ValueError when cursor.py is not available.

        Returns instance of CMySQLCursor or subclass.

        :param buffered: Return a buffering cursor.py
        :param raw: Return a raw cursor.py
        :param prepared: Return a cursor.py which uses prepared statements
        :param cursor_class: Use a custom cursor.py class
