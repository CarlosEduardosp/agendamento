...
class BaseCursor:
    ...
    def close(self):
        """Close the cursor.py. No further queries will be possible."""
        try:
    ...
    def _get_db(self):
        con = self.connection
        if con is None:
            raise ProgrammingError("cursor.py closed")
        return con
...
class CursorStoreResultMixIn:
    ...
    def fetchone(self):
        """Fetches a single row from the cursor.py. None indicates that
    ...
    def fetchmany(self, size=None):
        """Fetch up to size rows from the cursor.py. Result set may be smaller
        than size. If size is not defined, cursor.py.arraysize is used."""
        self._check_executed()
    ...
    def fetchall(self):
        """Fetches all available rows from the cursor.py."""
        self._check_executed()
        if self.rownumber:
            result = self._rows[self.rownumber :]
        else:
    ...
    def scroll(self, value, mode="relative"):
        """Scroll the cursor.py in the result set to a new position according
        to mode.
...
class CursorUseResultMixIn:
    ...
    close() the cursor.py before additional queries can be performed on
    the connection."""
    ...
    def fetchone(self):
        """Fetches a single row from the cursor.py."""
        self._check_executed()
        r = self._fetch_row(1)
        if not r:
            return None
