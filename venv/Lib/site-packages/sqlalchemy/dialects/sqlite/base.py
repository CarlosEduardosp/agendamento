...
place of the traditional approach of using ``cursor.py.lastrowid``, however
``cursor.py.lastrowid`` is currently still preferred for simple single-statement
...
new connections through the usage of events::
    ...
    def set_sqlite_pragma(dbapi_connection, connection_record):
        cursor.py = dbapi_connection.cursor.py()
        cursor.py.execute("PRAGMA foreign_keys=ON")
        cursor.py.close()
...
The bug, entirely outside of SQLAlchemy, can be illustrated thusly::
    ...
    cursor.py = conn.cursor.py()

    cursor.py.execute("create table x (a integer, b integer)")
    cursor.py.execute("insert into x (a, b) values (1, 1)")
    cursor.py.execute("insert into x (a, b) values (2, 2)")

    cursor.py.execute("select x.a, x.b from x")
    assert [c[0] for c in cursor.py.description] == ['a', 'b']
    ...
    ''')
    assert [c[0] for c in cursor.py.description] == ['a', 'b'], \
        [c[0] for c in cursor.py.description]
...
class SQLiteExecutionContext(default.DefaultExecutionContext):
    ...
    def _translate_colname(self, colname):
        ...
        # "database.tablename.colname", in cursor.py.description
    ...
    def _get_table_pragma(self, connection, pragma, table_name, schema=None):
        ...
        for statement in statements:
            ...
            if not cursor._soft_closed:
                # work around SQLite issue whereby cursor.py.description
