...
class AsyncAdapt_aiomysql_cursor:
    ...
    def close(self):
        # note we aren't actually closing the cursor.py here,
        # we are just letting GC do it.   to allow this to be async
        # we would need the Result to change how it does "Safe close cursor.py".
        # MySQL "cursors" don't actually have state to be "closed" besides
        # exhausting rows, which we already have done for sync cursor.py.
        # another option would be to emulate aiosqlite dialect and assign
        # cursor.py only if we are doing server side cursor.py operation.
        self._rows[:] = []
