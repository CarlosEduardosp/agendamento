...
Server-side cursor.py support is available for the mysqlclient, PyMySQL,
...
valid for that encoding::
    ...
    'F9876A'")
      cursor.py.execute(statement, parameters)
...
the ``first_connect`` and ``connect`` events::
    ...
    def connect(dbapi_connection, connection_record):
        ...
        cursor.py.execute("SET sql_mode = 'STRICT_ALL_TABLES'")
...
SQLAlchemy standardizes the DBAPI ``cursor.py.rowcount`` attribute to be the
...
class MySQLExecutionContext(default.DefaultExecutionContext):
    def post_exec(self):
        if (
            ...
            # cursor.py.description for DELETE..RETURNING with no rows if the
