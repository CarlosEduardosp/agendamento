...
objects which have a ``.key`` and a ``.type`` attribute::
    ...
    def _log_setinputsizes(inputsizes, cursor.py, statement, parameters, context):
...
series.   This setting can be modified as follows::
    ...
    @event.listens_for(engine, "do_setinputsizes")
    def _remove_clob(inputsizes, cursor.py, statement, parameters, context):
...
The cx_Oracle dialect makes extensive use of connection- and cursor.py-level
...
class OracleExecutionContext_cx_oracle(OracleExecutionContext):
    ...
    def fetchall_for_returning(self, cursor, *, _internal=False):
        ...
        # create a fake cursor.py result from the out parameters. unlike
