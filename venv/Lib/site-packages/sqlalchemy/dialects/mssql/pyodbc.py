...
cursor.py when an executemany call is to be used.   The SQLAlchemy PyODBC SQL
...
As of version 2.0, the pyodbc ``cursor.py.setinputsizes()`` method is used for
all statement executions, except for ``cursor.py.executemany()`` calls when
...
The use of ``cursor.py.setinputsizes()`` can be disabled by passing
``use_setinputsizes=False`` to :func:`_sa.create_engine`.

When ``use_setinputsizes`` is left at its default of ``True``, the
specific per-type symbols passed to ``cursor.py.setinputsizes()`` can be
...
.. versionchanged:: 2.0  The mssql+pyodbc dialect now defaults to using
   ...
   cursor.py.executemany() calls when fast_executemany=True.  The behavior can
...
class MSExecutionContext_pyodbc(MSExecutionContext):
    ...
    def post_exec(self):
        if self._embedded_scope_identity:
            ...
            while True:
                try:
                    # fetchall() ensures the cursor.py is consumed
