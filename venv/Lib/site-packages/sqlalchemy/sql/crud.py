        ...
        if col_key in parameters and col_key not in check_columns:
            ...
            if c.primary_key and need_pks:
                ...
                if implicit_returning:
                    ...
                    # we can use cursor.py.lastrowid.
...
def _append_param_insert_pk_no_returning(compiler, stmt, c, values, kw):
    ...
    or we may use cursor.py.lastrowid if supported by the dialect.
    ...
    if (
        ...
        and (
            # dialect can't use cursor.py.lastrowid
        ...
        # dialect supports cursor.py.lastrowid, switch on the lastrowid flag so
        # that the DefaultExecutionContext calls upon cursor.py.lastrowid
        compiler.postfetch_lastrowid = True
...
def _get_returning_modifiers(compiler, stmt, compile_state, toplevel):
    ...
    # check if we have access to simple cursor.py.lastrowid.  we can use that
