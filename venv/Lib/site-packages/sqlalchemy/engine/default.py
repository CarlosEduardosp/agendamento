...
class DefaultExecutionContext(ExecutionContext):
    ...
    def _execute_scalar(self, stmt, type_, parameters=None):
        """Execute a string statement on the current cursor.py, returning a
        scalar result.
    ...
    def get_lastrowid(self):
        """return self.cursor.py.lastrowid, or equivalent, after an INSERT.

        This may involve calling special cursor.py functions, issuing a new SELECT
        on the cursor.py (or a new one), or returning a stored value that was
    ...
    def _setup_dml_or_text_result(self):
        ...
        if self.isinsert:
            if (
                ...
                strategy = _cursor.FullyBufferedCursorFetchStrategy(
                    ...
                    # maintain alt cursor.py description if set by the
        ...
        if cursor_description is None:
            ...
            # strip out the sentinel columns from cursor.py description
        ...
        if self.isinsert:
            if self._is_implicit_returning:
                ...
                # test that it has a cursor.py metadata that is accurate. the
            ...
            # test that it has a cursor.py metadata that is accurate.
            # the rows have all been fetched however.
            assert result._metadata.returns_rows

        elif not result._metadata.returns_rows:
            # no results, get rowcount
            # (which requires open cursor.py on some drivers)
            result.rowcount
            self._has_rowcount = True
            result._soft_close()
    ...
    def _prepare_set_input_sizes(
        ...
        """Given a cursor.py and ClauseParameters, prepare arguments
        in order to call the appropriate
        style of ``setinputsizes()`` on the cursor.py, using DB-API types
    ...
    def _exec_default_clause_element(self, column, default, type_):
        ...
        # cursor.py.execute() sequence, since we don't want to modify the state
