...
"""Define cursor.py-specific result set constructs including
:class:`.CursorResult`."""
...
"""integer index in cursor.py.description
...
"""name that is usually in cursor.py.description
...
"""raw name from cursor.py.description"""
...
class CursorResultMetaData(ResultMetaData):
    ...
    def __init__(
        ...
        # merge cursor.py.description with the column info
        ...
        # to use names in cursor.py.description in some cases we need to have
    ...
    def _merge_cursor_description(
        ...
        """Merge a cursor.py.description with compiled result column information.
        ...
        cursor.py.description.  In this case, we assume a 1-1 positional mapping
        between the entries in cursor.py.description and the compiled object.
        This is also the most performant case as we disregard extracting /
        decoding the column names present in cursor.py.description since we
        ...
        names from cursor.py.description and index those as the primary
        ...
        merge the cursor.py.description and the compiled construct's information
        positionally, and warn if there are additional description names
        present, however we still decode the names in cursor.py.description
        as we don't have a guarantee that the names in the columns match
        on these.   In the unordered case, we match names in cursor.py.description
        to that of the compiled construct based on name matching.
        In both of these cases, the cursor.py.description names and the column
        expression objects and names are indexed as result row target keys.

        The final case is much less common, where we have a compiled
        non-textual SQL expression construct, but the number of columns
        in cursor.py.description doesn't match what's in the compiled
        ...
        if (
            ...
            # the names from cursor.py.description
            ...
            # to read cursor.py.description names
    ...
    def _colnames_from_description(self, context, cursor_description):
        """Extract column names and data types from a cursor.py.description.
    ...
    def _create_description_match_map(
        ...
        """when matching cursor.py.description to a set of names that are present
        in a Compiled object, as is the case with TextualSelect, get all the
        names we expect might match those in cursor.py.description.
        """
        ...
        for ridx, elem in enumerate(result_columns):
            ...
            if key in d:
                ...
                # name in the cursor.py description, this will allow all of those
...
class NoCursorFetchStrategy(ResultFetchStrategy):
    """Cursor strategy for a result that has no open cursor.py.

    There are two varieties of this strategy, one for DQL and one for
    DML (and also DDL), each of which represent a result that had a cursor.py
...
class NoCursorDQLFetchStrategy(NoCursorFetchStrategy):
    """Cursor strategy for a DQL result that has no open cursor.py.
    ...
    where the cursor.py is closed and no rows remain available.  The owning result
...
class NoCursorDMLFetchStrategy(NoCursorFetchStrategy):
    """Cursor strategy for a DML result that has no open cursor.py.
...
class CursorFetchStrategy(ResultFetchStrategy):
    """Call fetch methods from a DBAPI cursor.py.
...
class BufferedRowCursorFetchStrategy(CursorFetchStrategy):
    """A cursor.py fetch strategy with row buffering behavior.

    This strategy buffers the contents of a selection of rows
    before ``fetchone()`` is called.  This is to allow the results of
    ``cursor.py.description`` to be available immediately, when
...
class FullyBufferedCursorFetchStrategy(CursorFetchStrategy):
    """A cursor.py strategy that buffers rows fully upon creation.
...
class CursorResult(Result[_T]):
    """A Result that is representing state from a DBAPI cursor.py.
    ...
    def _soft_close(self, hard=False):
        ...
        This releases all DBAPI cursor.py resources, but leaves the
    ...
    def splice_vertically(self, other):
        ...
        the rows of this cursor.py result. The incoming :class:`.CursorResult`
    ...
    @property
    def lastrowid(self):
        """Return the 'lastrowid' accessor on the DBAPI cursor.py.
    ...
    def returns_rows(self):
        ...
        always be synonymous with whether or not the DBAPI cursor.py had a
        ``.description`` attribute, indicating the presence of result columns,
        noting that a cursor.py that returns zero rows still has a
    ...
    def close(self) -> Any:
        ...
        This closes out the underlying DBAPI cursor.py corresponding to the
        statement execution, if one is still present.  Note that the DBAPI
        cursor.py is automatically released when the :class:`_engine.CursorResult`
