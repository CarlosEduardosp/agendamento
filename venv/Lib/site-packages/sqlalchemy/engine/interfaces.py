...
class ExecuteStyle(Enum):
    """indicates the :term:`DBAPI` cursor.py method that will be used to invoke
    a statement."""

    EXECUTE = 0
    """indicates cursor.py.execute() will be used"""

    EXECUTEMANY = 1
    """indicates cursor.py.executemany() will be used."""

    INSERTMANYVALUES = 2
    """indicates cursor.py.execute() will be used with an INSERT where the
...
class DBAPICursor(Protocol):
    """protocol representing a :pep:`249` database cursor.py.
    ...
    def description(
        ...
        .. seealso::
            ...
            `cursor.py.description <https://www.python.org/dev/peps/pep-0249/#description>`_
            - in :pep:`249`
...
class Dialect(EventTarget):
    ...
    """either the 'tuple' or 'list' type, depending on what cursor.py.execute()
    ...
    cursor.py.lastrowid tends to be more performant on most backends.
    ...
    automatic cursor.py.setinputsizes() calls.
    ...
    def do_set_input_sizes(
        ...
        """invoke the cursor.py.setinputsizes() method with appropriate arguments
    ...
    def do_executemany(
        ...
        """Provide an implementation of ``cursor.py.executemany(statement,
        parameters)``."""
    ...
    def do_execute(
        ...
        """Provide an implementation of ``cursor.py.execute(statement,
        parameters)``."""
    ...
    to an :class:`_engine.Engine` object might look like::
        ...
        class LogCursorEventsPlugin(CreateEnginePlugin):
            ...
            def _log_event(
                ...
                self.log.info("Plugin logged cursor.py event: %s", statement)
...
class ExecutionContext:
    ...
    cursor: DBAPICursor
    """DB-API cursor.py procured from the connection"""
    ...
    list corresponds to a ``cursor.py.execute()`` call and a multiple-element
    list corresponds to ``cursor.py.executemany()``, except in the case
    of :attr:`.ExecuteStyle.INSERTMANYVALUES` which will use
    ``cursor.py.execute()`` one or more times.
    ...
    """the style of DBAPI cursor.py method that will be used to execute
    a statement.
    ...
    Historically this attribute links to whether ``cursor.py.execute()`` or
    ``cursor.py.executemany()`` will be used.  It also can now mean that
    "insertmanyvalues" may be used which indicates one or more
    ``cursor.py.execute()`` calls.
    ...
    def create_cursor(self) -> DBAPICursor:
        """Return a new cursor.py generated from this ExecutionContext's
        connection.

        Some dialects may wish to change the behavior of
        connection.cursor.py(), such as postgresql which may return a PG
        "server side" cursor.py.
        """
    ...
    def get_out_parameter_values(
        ...
        """Return a sequence of OUT parameter values from a cursor.py.
    ...
    def get_rowcount(self) -> Optional[int]:
        """Return the DBAPI ``cursor.py.rowcount`` value, or in some
    ...
    def fetchall_for_returning(self, cursor: DBAPICursor) -> Sequence[Any]:
        """For a RETURNING result, deliver cursor.py.fetchall() from the
        DBAPI cursor.py.
...
class AdaptedConnection:
    ...
    of the asyncio connection/cursor.py API provided by the driver.
